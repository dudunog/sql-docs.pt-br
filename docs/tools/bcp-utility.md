---
title: Utilitário bcp
description: O utilitário bcp (programa de cópia em massa) copia em massa os dados entre uma instância do SQL Server e um arquivo de dados em um formato especificado pelo usuário.
ms.prod: sql
...
>
> Para obter informações detalhadas sobre como usar o bcp com o Azure Synapse Analytics, confira [Carregar dados com o bcp](/azure/sql-data-warehouse/sql-data-warehouse-load-with-bcp).
...
![Ícone de link do tópico](../database-engine/configure-windows/media/topic-link.gif "Ícone de link do tópico") Para obter as convenções de sintaxe usadas para a sintaxe de **bcp**, confira [Convenções da sintaxe Transact-SQL &#40;Transact-SQL&#41;](../t-sql/language-elements/transact-sql-syntax-conventions-transact-sql.md).  
...
## <a name="download-the-latest-version-of-bcp-utility"></a>Baixe a versão mais recente do utilitário bcp

**[![Baixar bcp para x64](../ssdt/media/download.png) Baixar Utilitários de Linha de Comando 15 da Microsoft para SQL Server (x64)](https://go.microsoft.com/fwlink/?linkid=2142258)**
<br>**[![Baixar bcp para x86](../ssdt/media/download.png) Baixar Utilitários de Linha de Comando 15 da Microsoft para SQL Server (x86)](https://go.microsoft.com/fwlink/?linkid=2142257)**

As ferramentas de linha de comando estão em GA (disponibilidade geral), no entanto, elas estão sendo lançadas com o pacote do instalador para [!INCLUDE[sql-server-2019](../includes/sssqlv15-md.md)].

### <a name="version-information"></a>Informações sobre versão

Número da versão: 15.0.2 <br>
...
Esse componente exige o [Windows Installer 4.5](https://www.microsoft.com/download/details.aspx?id=8483) e o [Microsoft Driver ODBC 17 para SQL Server](https://aka.ms/downloadmsodbcsql).

Para verificar a versão do BCP, execute o comando `bcp /v` e confirme se a 15.0.2000.5 ou posterior está em uso.
...
|*code_page*|Um número de página de código específico, por exemplo, 850.<br /><br /> As versões anteriores à versão 13 ([!INCLUDE[ssSQL15](../includes/sssql16-md.md)]) não dão suporte à página de código 65001 (codificação UTF-8). As versões a partir da 13 podem importar a codificação UTF-8 para versões anteriores do [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)].|  
|||
  
 **-d** _**database\_name**_<a name="d"></a>   
 Especifica o banco de dados que deve ser conectado. Por padrão, bcp.exe se conecta ao banco de dados padrão do usuário. Se -d database_name e um nome de três partes (database_name.schema.table, passada como o primeiro parâmetro para bcp.exe) estiverem especificados, ocorrerá um erro porque você não pode especificar o nome do banco de dados duas vezes. Se *database_name* começar com um hífen (-) ou uma barra (/), não adicione um espaço entre **-d** e o nome do banco de dados.  

**-D**<a name="D"></a>  
Faz com que o valor passado para a opção `-S` do `bcp` seja interpretada como um DNS (nome da fonte de dados). Um DSN pode ser usado para inserir opções de driver para simplificar as linhas de comando, impor opções de driver que não seriam acessíveis de outra forma usando linha de comando, como MultiSubnetFailover, ou para ajudar a proteger credenciais confidenciais contra a detecção como argumentos de linha de comando. Para obter mais informações, confira _Suporte para DSN no sqlcmd e no bcp_ em [Conectar-se com sqlcmd](../connect/odbc/linux-mac/connecting-with-sqlcmd.md).
...
**-G**<a name="G"></a>
 ...
 Essa opção é usada pelo cliente ao se conectar ao Banco de Dados SQL do Azure ou o Azure Synapse Analytics para especificar que o usuário seja autenticado usando a autenticação do Azure Active Directory. A opção -G exige a [versão 14.0.3008.27 ou posterior](https://go.microsoft.com/fwlink/?LinkID=825643). Para determinar a versão, execute bcp -v. Para obter mais informações, confira [Usar a Autenticação do Azure Active Directory para a autenticação no Banco de Dados SQL ou no Azure Synapse Analytics](/azure/sql-database/sql-database-aad-authentication). 

> [!IMPORTANT]
> A opção **- G** só se aplica ao Banco de Dados SQL e ao Azure Data Warehouse.
> No momento, a autenticação interativa e integrada ao AAD não é compatível com o Linux nem com o macOS.

> [!TIP]
> Para verificar se a sua versão do bcp inclui suporte para a Autenticação do AAD (Azure Active Directory), digite **bcp --** (bcp\<space>\<dash>\<dash>) e verifique se você vê -G na lista de argumentos disponíveis.
...
- **Integração ao Azure Active Directory**
    ...
    Para a autenticação integrada do Azure Active Directory, forneça a opção **-G** sem um nome de usuário e uma senha. Essa configuração pressupõe que a conta de usuário do Windows atual (a conta em que o comando bcp está sendo executado) é federada com o Azure AD:
...
- **Interatividade do Azure Active Directory**  
   ...
   O Azure AD interativo requer **bcp** [versão 15.0.1000.34](#download-the-latest-version-of-bcp-utility) ou posterior, bem como [ODBC versão 17.2 ou posterior](https://aka.ms/downloadmsodbcsql).  

   Para habilitar a autenticação interativa, forneça a opção -G com o nome de usuário (-U) apenas, sem uma senha.

   O exemplo a seguir exporta dados usando o modo interativo do Azure AD que indica o nome de usuário em que o usuário representa uma conta do AAD. Este é o mesmo exemplo usado na seção anterior: *Nome de usuário e senha do Azure Active Directory*.  
   ...
   Se houver usuários convidados em um Azure AD específico e eles fizerem parte de um grupo existente do Banco de Dados SQL que tenha permissões de banco de dados para executar o comando bcp, o alias de usuário convidado deles será usado (por exemplo, *keith0@adventureworks.com* ).
  
**-h** _**"load hints**_[ ,... *n*] **"** <a name="h"></a> Especifica a dica ou dicas a serem usadas durante uma importação em massa de dados em uma tabela ou exibição.  
...
Especifica que um bloqueio no nível de tabela de atualização em massa é adquirido durante a operação de carregamento em massa, caso contrário, um bloqueio no nível de linha é adquirido. Essa dica melhora significativamente o desempenho porque manter um bloqueio durante a operação de cópia em massa reduz a contenção de bloqueios na tabela. Uma tabela pode ser carregada simultaneamente através de vários clientes se não tiver nenhum índice e **TABLOCK** for especificado. Por padrão, o comportamento de bloqueio é determinado pela opção de tabela **bloqueio de tabela em carregamento em massa**.  
  ...
  **CHECK_CONSTRAINTS**  
  ...
  > [!NOTE]
  > Agora, o **bcp** impõe validação de dados e verificações de dados que poderão provocar falhas em scripts se forem executados em dados inválidos em um arquivo de dados.
...
- O utilitário bcp também pode ser baixado separadamente do [Feature Pack do Microsoft SQL Server 2016](https://www.microsoft.com/download/details.aspx?id=56833).  Selecione `ENU\x64\MsSqlCmdLnUtils.msi` ou `ENU\x86\MsSqlCmdLnUtils.msi`.
...
- [Usar caracteres especiais adicionais](/windows-server/administration/windows-commands/set_1#remarks)

    Os caracteres <, >, |, &, ^ são especiais do Shell de comando e devem ser precedidos pelo caractere de escape (^) ou colocados entre aspas quando usados na cadeia de caracteres (por exemplo, "StringContaining&Symbol"). Se você usar aspas para incluir uma cadeia de caracteres que contenha um dos caracteres especiais, as aspas serão definidas como parte do valor da variável de ambiente.
...
## <a name="data-validation"></a>Validação de dados
 ...
 Agora, o **bcp** impõe validação de dados e verificações de dados que poderão provocar falhas em scripts se forem executados em dados inválidos em um arquivo de dados. Por exemplo, o **bcp** agora verifica se:  
...
C. Copiando linhas de tabela em um arquivo de dados (com autenticação mista)
...
J. Especificando uma página de código
...
- **Básico** Este exemplo cria um arquivo de dados nomeado `StockItemTransactions_character.bcp` e copia os dados da tabela nesse arquivo usando o formato de **caractere**.
...
- **Expandido** Este exemplo cria um arquivo de dados chamado `StockItemTransactions_native.bcp` e copia os dados da tabela para ele usando o formato **nativo**.  O exemplo também: especifica o número máximo de erros de sintaxe, um arquivo de erro e um arquivo de saída.
...
- **Básico** Este exemplo usa o arquivo de dados `StockItemTransactions_character.bcp` criado anteriormente.
...
- **Expandido** Este exemplo usa o arquivo de dados `StockItemTransactions_native.bcp` criado anteriormente.  O exemplo também: usa a dica **TABLOCK**, especifica o tamanho do lote, o número máximo de erros de sintaxe, um arquivo de erro e um arquivo de saída.
...
Para copiar o conjunto de resultados de uma instrução Transact-SQL em um arquivo de dados, use a opção **queryout** .  O exemplo a seguir copia os nomes da tabela `WideWorldImporters.Application.People` , classificados pelo nome completo, no arquivo de dados `People.txt` .  Observação: a opção **-t** é usada para criar um arquivo delimitado por vírgula.
