...
Este artigo descreve como consultar colunas criptografadas com o [Always Encrypted](../../../relational-databases/security/encryption/always-encrypted-database-engine.md) usando o [Azure Data Studio](../../../azure-data-studio/what-is-azure-data-studio.md). Com o Azure Data Studio, você pode:
...
![Captura de tela da consulta SELECT * FROM [dbo].[Patients] e os resultados da consulta mostrados como valores binários de texto cifrado.](../../../relational-databases/security/encryption/media/always-encrypted-ads-query-ciphertext.png)
...
![Captura de tela da consulta SELECT * FROM [dbo].[Patients] e os resultados da consulta mostrados como valores de texto sem formatação.](../../../relational-databases/security/encryption/media/always-encrypted-ads-query-plaintext.png)
...
![Captura de tela da consulta DECLARE @SSN char(11) = '795-73-9838' SELECT * FROM [dbo].[Patients] WHERE [SSN] = @SSN e os resultados da consulta.](../../../relational-databases/security/encryption/media/always-encrypted-ads-query-parameters.png)
...
3. Clique em **OK** para fechar as **Propriedades avançadas**.
...
Para executar instruções que aproveitam um enclave seguro no lado do servidor quando o [Always Encrypted com enclaves seguros](always-encrypted-enclaves.md), você precisará especificar um protocolo e uma URL de atestado de enclave, além de habilitar o Always Encrypted para a conexão. Para obter informações detalhadas, confira [Pré-requisitos para executar instruções T-SQL usando enclaves no Azure Data Studio](always-encrypted-enclaves-query-columns.md#prerequisites-for-running-t-sql-statements-using-enclaves-in-azure-data-studio).
