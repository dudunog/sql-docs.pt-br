...
-   @closed_high_end_point = 0  
     ...
     Somente as linhas na cdc.capture_instance_CT tabela de alteração que têm uma hora de confirmação associada estritamente menor que end_time são incluídas no conjunto de resultados.  
...
## <a name="remarks"></a>Comentários  
 O fn_all_changes_<capture_instance função> serve como um wrapper para a função de consulta cdc.fn_cdc_get_all_changes_<capture_instance>. O procedimento armazenado sys.sp_cdc_generate_wrapper é usado para gerar o script para criar o wrapper.  
