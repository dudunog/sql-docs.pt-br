...
Essa configuração está disponível apenas no Azure Synapse Analytics.
...
> [!NOTE]
> No [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)], a configuração no escopo do banco de dados MAXDOP para novos bancos de dados de pool único e elástico é definida como 8 por padrão. MAXDOP pode ser configurado para cada banco de dados, conforme descrito no artigo atual. Para obter recomendações sobre a configuraração ideal do MAXDOP, confira a seção [Recursos Adicionais](#additional-resources).
...
**APLICA-SE A**: [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] (Começando pelo [!INCLUDE[ssSQL16](../../includes/sssql16-md.md)]) e [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]
...
- No [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] e na Instância Gerenciada de SQL do Azure, essa opção é definida em `TempDB`, e a configuração dos bancos de dados de usuário individuais não tem nenhum efeito.
...
- ["I smell a parameter!"](/archive/blogs/queryoptteam/i-smell-a-parameter) (Sinto cheiro de parâmetro!)
