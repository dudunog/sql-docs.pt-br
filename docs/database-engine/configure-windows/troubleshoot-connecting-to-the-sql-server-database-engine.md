---
title: Solucionar problemas na conexão com o Mecanismo de Banco de Dados do SQL Server
description: Descubra como solucionar problemas de conexão. Confira as etapas a serem seguidas quando não for possível usar TCP/IP para se conectar a um Mecanismo de Banco de Dados do SQL Server em um servidor único.
ms.prod: sql
...
Este artigo lista as técnicas de solução de problemas a ser usadas quando você não pode se conectar a uma instância do Mecanismo de Banco de Dados do SQL Server em um servidor único.
...
- Este tópico não inclui informações sobre erros do SSPI. Para erros do SSPI, confira [Como solucionar a mensagem de erro "Não é possível gerar contexto de SSPI"](https://support.microsoft.com/kb/811889).
...
- Este tópico não inclui informações sobre a conectividade do Banco de Dados SQL do Azure. Para obter ajuda, consulte [Solução de problemas de conectividade com o Banco de Dados SQL do Microsoft Azure](/azure/sql-database/troubleshoot-connectivity-issues-microsoft-azure-sql-database).
...
3. No painel esquerdo, selecione **Serviços do SQL Server**.
...
Para se conectar a uma instância nomeada, o serviço do SQL Server Browser deve estar em execução. No Configuration Manager, localize o serviço **SQL Server Browser** e verifique se ele está em execução. Inicie, caso não esteja em execução. O serviço do SQL Server Browser não é necessário para instâncias padrão.
...
Esse procedimento usa o SQL Server Management Studio. Se você não tiver o Management Studio instalado, confira [Baixar o SSMS (SQL Server Management Studio)](../../ssms/download-sql-server-management-studio-ssms.md). Se não conseguir instalar o Management Studio, você poderá testar a conexão usando o utilitário `sqlcmd.exe`. `sqlcmd.exe` é instalado com o Mecanismo de Banco de Dados. Para saber mais sobre o `sqlcmd.exe`, consulte o [Utilitário sqlcmd](../../tools/sqlcmd-utility.md).)

1. Entre no computador em que o SQL Server está instalado, usando um logon com permissão para acessar o SQL Server. (Durante a instalação, o SQL Server requer que pelo menos um logon seja especificado como Administrador do SQL Server. Se você não conhecer um administrador, confira [Conectar-se ao SQL Server quando os administradores do sistema estiverem bloqueados](connect-to-sql-server-when-system-administrators-are-locked-out.md).)
...
3. Na caixa de diálogo **Conectar ao Servidor** , na caixa de tipo **Servidor** , selecione **Mecanismo de Banco de Dados**. Na caixa **Autenticação** , selecione **Autenticação do Windows**. Na caixa **Nome do servidor**, digite um dos seguintes tipos de conexão:
   ...
   Se você receber um erro neste ponto, precisará resolvê-lo antes de continuar. Há muitas possibilidades que podem representar um problema. Seu logon pode não estar autorizado a se conectar. O banco de dados padrão pode estar ausente.
...
5. Se você puder se conectar usando memória compartilhada, teste a conexão usando TCP. Você pode forçar uma conexão TCP especificando `tcp:` antes do nome. Por exemplo:
...
6. Se você puder se conectar à memória compartilhada, mas não ao TCP, você deve corrigir o problema do TCP. O problema mais provável é que o TCP não está habilitado. Para habilitar o TCP, consulte as etapas para [Habilitar protocolos](#enableprotocols) acima.

7. Se sua meta for se conectar a uma conta que não é uma conta de administrador, depois que você puder se conectar como administrador, tente realizar a conexão novamente usando o logon de Autenticação do Windows ou o logon de Autenticação do SQL Server que o aplicativo cliente usará.
...
2. No prompt de comando, digite `ipconfig` e pressione Enter. Anote os endereços **IPv4** e **IPv6** .
...
Em algumas instalações do SQL Server, a conexão ao Mecanismo de Banco de Dados de outro computador não está habilitada, a menos que o administrador use o Configuration Manager para habilitar. Para habilitar conexões de outro computador:

1. Abra o SQL Server Configuration Manager como descrito anteriormente.
2. Usando o Configuration Manager, no painel esquerdo, expanda **Configuração de Rede do SQL Server** e selecione a instância do SQL Server que você deseja se conectar. O painel direito lista os protocolos de conexão disponíveis. Normalmente, a memória compartilhada está habilitada. Ele só pode ser usado no mesmo computador, por isso a maioria das instalações deixam a memória compartilhada habilitada. Para conectar-se ao SQL Server de outro computador, você normalmente usa o TCP/IP. Se o TCP/IP não estiver habilitado, clique com o botão direito do mouse em **TCP/IP** e clique em **Habilitar**.
...
Se você estiver se conectando a uma instância nomeada ou uma porta diferente da porta TCP 1433, também deverá abrir a porta 1434 UDP para o serviço de Navegador do SQL Server. Para obter instruções passo a passo para configurar o Firewall do Windows, confira [Configurar um Firewall do Windows para acesso ao Mecanismo de Banco de Dados](configure-a-windows-firewall-for-database-engine-access.md).
...
2. Depois de se conectar usando o endereço IP e o número da porta, tente se conectar usando o endereço IP sem um número de porta. Para uma instância padrão, basta usar o endereço IP. Para uma instância nomeada, use o endereço IP e o nome da instância no formato endereço IP barra invertida nome da instância, por exemplo, `192.168.1.101\<instance name>`. Se isso não funcionar, você provavelmente tem um dos seguintes problemas:
   ...
   - Se você estiver se conectando à instância padrão, ele poderá estar escutando em uma porta diferente da porta 1433 TCP, e o cliente estará tentando se conectar ao número da porta incorreto.
   - Se você estiver se conectando a uma instância nomeada, o número da porta não estará sendo retornado ao cliente.
